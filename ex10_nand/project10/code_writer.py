...
class CompiliationEngine:
    ...
    def compileExpression(self):
        ...
        priTokenType2 = self.inpotTokens.tokenType(self.inpotTokens.output_token[self.counter + 1])
        while token not in [',', ')', ']', ';']:
            if token not in OP_TYPES or (token in UNARY_OP_TYPES and priTokenType != 'IDENTIFIER'):# and priTokenType2 != "INT_CONST"):
                self.compileTerm()
                #token = self.inpotTokens.output_token[self.counter]
            else:
                self.outputFile.write(OPEN_SYMBOL + self.checkSymbol(token) + CLOSE_SYMBOL)
